  ...
  <div class="max-w-6xl w-full mx-auto px-4 pb-8 sm:pb-12 flex-grow flex flex-col">
    ...
    >
      <div>
        ...
        >
          {{ $t('recipes.title', 'Entdecken') }}
        </h1>
        <p class="text-gray-500 text-lg max-w-xl relative z-10">
          {{
            $t(
              'recipes.subtitle',
              'Lass dich inspirieren und finde neue Lieblingsrezepte für deine Woche.',
            )
          }}
        </p>
      </div>
    ...
    <!-- Forces vertical stacking: Full width search on top, grouped filters below -->
    <div
      class="sticky top-[72px] md:top-[80px] z-40 bg-bg-cream/95 backdrop-blur-md py-5 mb-8 border-b border-gray-200"
    >
      <div class="flex flex-col gap-6 w-full">
        <!-- Search Input (Full Width on all screens) -->
        <div class="relative w-full shrink-0 group">
          <!-- Search Icon: Bulletproof absolute centering -->
          <svg
          ...
          <input
            ...
            type="text"
            :placeholder="$t('recipes.search_placeholder', 'Rezepte oder Zutaten suchen...')"
            class="block w-full pl-12 pr-4 py-3.5 border border-gray-200 rounded-2xl leading-5 bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-green focus:border-primary-green transition-all shadow-sm text-gray-700"
          />
        </div>
        ...
        <div class="w-full flex flex-col md:flex-row gap-6 md:gap-12">
          ...
          <div v-for="group in categoryGroups" :key="group.titleKey" class="flex flex-col gap-3">
            <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider px-1">
              {{ $t(group.titleKey) }}
            </h4>
            <!-- flex-wrap allows items to gracefully drop to the next line on small screens -->
            <div class="flex flex-wrap gap-2.5">
              <button
                ...
                :class="[
                  'whitespace-nowrap shrink-0 px-4 py-2 rounded-2xl text-sm font-semibold transition-all duration-200 border flex items-center gap-2',
    ...
    <!-- 4. States: Loading -->
    <div v-if="isLoading" class="flex flex-col justify-center items-center py-24 text-gray-400">
      <svg
    ...
    >
      ...
      <h3 class="text-2xl font-bold text-dark-green mb-2">
        {{ $t('recipes.no_results', 'Keine Rezepte gefunden') }}
      </h3>
      <p class="text-gray-500 max-w-md mx-auto">
        {{
          $t(
            'recipes.no_results_desc',
            'Probiere es mit einem anderen Suchbegriff oder passe deine Filter an.',
          )
        }}
      </p>
      <button
    ...
    <!-- 5. Recipe Grid -->
    <!-- TransitionGroup enables smooth filtering animations without breaking the CSS grid -->
    <TransitionGroup
      v-else
      name="recipe-list"
      tag="div"
    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- Save/Favorite Button Placeholder -->
          <button
            @click.prevent
        ...
        <div class="p-6 flex-grow flex flex-col">
          ...
          <div class="mt-auto flex flex-col gap-4">
            ...
            <!-- Time Info (Synced layout with LandingView for UI consistency) -->
            <div
...
// Grouped categories mapping to i18n keys for better UX and translation support
...
// Timeout reference for debouncing the search input
let searchTimeout: ReturnType<typeof setTimeout> | null = null

// Watcher: Waits 400ms after the user stops typing before triggering the API call
watch(searchQuery, () => {
...
const fetchRecipes = async () => {
  ...
  try {
    // Construct dynamic endpoint URL based on active search/filter parameters
    ...
    // Support both paginated structures (response.data.data) and flat arrays (response.data)
...
<style scoped>
/*
  TransitionGroup Styles - Grid Safe!
  By omitting 'position: absolute' on the leave-active class, the layout grid remains intact during animations.
*/
.recipe-list-move,
